...
CONSUME_ROUTING_KEY = "recognizer_queue"
RESULT_ROUTING_KEY = "record_queue"
CONSUME_QUEUE = "recognizer_queue"
RESULT_QUEUE = "record_queue"
credentials = pika.PlainCredentials("guest", "guest")
parameters = pika.ConnectionParameters(
    "localhost", 5672, connection_attempts=3, credentials=credentials
)
...
logging.basicConfig(
    filename="./consumer_app.log", level=logging.INFO, format=LOG_FORMAT
)
...
def recognize(channel, method_frame, header_frame, body) -> None:
    ...
    channel.basic_publish(
        ...
        pika.BasicProperties(
            content_type="application/json", delivery_mode=pika.DeliveryMode.Transient
        ),
    )
    pass
